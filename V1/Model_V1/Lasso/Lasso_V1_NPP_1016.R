...
df_in<- read.csv("XY_MFG_V1_right_0918.csv")

# Define the strings to be removed
strings_to_remove <- c("_proc_0.5_MFG", "_tal_nlin_sym", "2011_17Networks_MNI152_FreeSurferConformed1mm_LooseMask",
                       "2011_17Networks_MNI152_FreeSurferConformed1mm_LiberalMask")

# Remove the specified strings from the column names
for (string in strings_to_remove) {
  colnames(df_in) <- gsub(string, "", colnames(df_in))
}

df <- as.data.table(df_in)

# Data check
names(df)[1:20]
dim(df)	#438571    452

# Get the length of unique values for each column
length(unique(df[, sub.id]))  # 8
length(unique(df[, XYZ_key]))  # 249807
length(unique(df[, ijk_key]))  # 33280 ???? Check why the difference ???

# Prepare group variables for predictors
df_in$sub.id <- as.factor(df_in$sub.id)  # Convert sub.id to factor
class(df_in$sub.id)  # "factor"
...
# Save the resulting df.std to a new CSV file
#write.csv(df.std, "XY_MFG_V1_right_std_1021.csv", row.names = FALSE)
...
training.samples = defineTrain(nrow(df),0.8)  # get 80% of df as training set # n=438571
...
sum(is.na(apply(df_in[,-c(1,2,3,4)], 1, sum)))  # 0
check.col.na = apply(df_in[,-c(1,2,3)], 2, sum); length(check.col.na); sum(is.na(check.col.na)) # 449 check!   0
...
#### Factor analysis for significant predictors ####

# Determine the number of factors to extract
# You can use various methods to determine the number of factors, such as scree plot or parallel analysis.
# For simplicity, let's assume we want to extract 3 factors.

num_factors <- 3

# Perform factor analysis
factor_analysis_result <- factanal(df_x.next, factors = num_factors, rotation = "varimax", start = NULL)
...
upper_tri <- r[upper.tri(r)] 
total_pairs <- length(upper_tri) #406
num_high_corr_pairs <- sum(upper_tri > 0.8) #75
percent_high_corr_pairs <- (num_high_corr_pairs / total_pairs) * 100 #18.4
...
model.lm <- lm( QNP_obs ~ ., data = df_next.lm)
summary(model.lm)
