...
y <- data.matrix(log(df_y))
x <- data.matrix(df_x)

t_y <- data.frame(t(df_y))
hist(t_y$MFG, breaks = 50)
qqnorm(t_y$IPL)
boxplot(t_y$MFG, horizontal = TRUE)

t_ylog <- log(t_y)
t_ylog$MFG <- log(t_y$MFG)
hist(t_ylog$MFG, breaks = 50)
qqnorm(t_ylog$MFG)
boxplot(t_ylog$MFG, horizontal = TRUE)

####### Run 

cv_model <- cv.glmnet(x, y, family = "mgaussian", nfolds = 5, alpha=1, trace.it = TRUE,standardize.response = TRUE)
print(cv_model)
