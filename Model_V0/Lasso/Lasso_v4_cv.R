...
factor_columns <- sapply(df_x, is.factor)
enc_df <- model.matrix(~.-1, data = df_x[, factor_columns])
num_columns <- df_x[, !factor_columns]
x_train <- cbind(enc_df, num_columns)
