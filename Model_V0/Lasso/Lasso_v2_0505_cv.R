...
set.seed(825)
cv_results <- caret::train(x = df[2:978],
                           y = df$QNP_obs,
...
cv_model <- cv.glmnet(x,y, family = "gaussian", nfolds = 10, alpha=0.1, trace.it = TRUE)
print(cv_model)
...
best_model = glmnet(x,y, alpha = 0.1, lambda=best_lambda, family = "gaussian")
coef(best_model)
